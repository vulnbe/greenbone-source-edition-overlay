...
RDEPEND="
	${DEPEND}
	>=net-analyzer/gvmd-${MVER}
	>=net-analyzer/ospd-openvas-${MVER}"
...
PATCHES=(
	# Remove uninstall-snippet that causes failing re-emerge.
	"${FILESDIR}/greenbone-security-assistant-21.4.1-uninstall-snippet.patch"
)
...
src_prepare() {
	...
	echo "--modules-folder ${MY_NODE_DIR}/${MY_NODE_N}" >> "${S}/${MY_PN}/.yarnrc" || die "echo failed"
	# Remove linter
	rm "${S}/${MY_PN}/.eslintrc.js"
...
src_configure() {
	# https://github.com/greenbone/gsa/issues/3039
	# "-DYARN_OFFLINE=true"
